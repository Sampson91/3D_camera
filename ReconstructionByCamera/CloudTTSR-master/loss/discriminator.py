...
class Discriminator(torch_neural_network.Module):
    ...
    def forward(self, train_crop_size_x4):
        ...
        train_crop_size_x4 = train_crop_size_x4.view(train_crop_size_x4.size(0), -1)
        train_crop_size_x4 = self.fully_connected_1(train_crop_size_x4)
        train_crop_size_x4 = self.LeakyReLU11(train_crop_size_x4)
...
if __name__ == '__main__':
    model = discriminator()
    out_size = torch.ones(1, 6, 100000)
    out = model(out_size)
    print(out.size())
