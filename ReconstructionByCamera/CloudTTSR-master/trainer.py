...
from cloudreader import cloud_save
import sys
import time
sys.path.append("dataset")
from cufed import cloud_upsample
from cufed import random_point_dropout
...
class Trainer():
    ...
    def train(self, current_epoch=0, is_init=False):
        ...
        for image_batch, sample_batched in enumerate(self.dataloader['train']):
            ...
            # calc loss
            is_print = ((image_batch + 1) % self.args.print_every == 0)  # flag of print
    ...
    def evaluate(self, current_epoch=0):
        """This model need to recode, now it's function is not effect"""
        self.logger.info(
        ...
        if (self.args.dataset == 'CUFED'):
            ...
            with torch.no_grad():
                ...
                for image_batch, sample_batched in enumerate(self.dataloader['test']):
                    count += 1
                    sample_batched = self.prepare(sample_batched)
                    ...
                    if (self.args.eval_save_results):
                        ...
                        super_resolution_save = np.transpose(
                            ...
                            (1, 0))
                        cloud_save(os.path.join(self.args.save_dir, 'save_results',
                                            str(image_batch).zfill(5) + '.obj'),
    ...
    def test(self):
        ...
        # LR and LR_sr
        self.obj = ReadObj(self)
        low_resolution_image = self.obj.cloud_read(self.args.lr_path)
        number_1 = low_resolution_image.shape[0]
        upsampled_low_resolution_image = cloud_upsample(low_resolution_image)

        # Ref and Ref_sr
        high_resolution_images_as_references = self.obj.cloud_read(self.args.ref_path)
        ...
        low_resolution_image_to_tensor = torch.from_numpy(
            low_resolution_image.transpose((1, 0))).unsqueeze(0).float().to(
            self.device)
        upsampled_low_resolution_image_to_tensor = torch.from_numpy(
            upsampled_low_resolution_image.transpose((1, 0))).unsqueeze(
        ...
        down_and_upsampled_Ref_image_to_tensor = torch.from_numpy(
            down_and_upsampled_Ref_image.transpose((1, 0))).unsqueeze(
            0).float().to(self.device)
        ...
        with torch.no_grad():
            ...
            #     super_resolution_save.squeeze().round().cpu().numpy(),
            #     (1, 0)).astype(np.uint8)
            super_resolution_save = np.transpose(super_resolution_save.squeeze().cpu().numpy(),(1, 0))
            save_path = os.path.join(self.args.save_dir, 'save_results',
                                    os.path.basename(self.args.lr_path))
            cloud_save(save_path, super_resolution_save)
    ...
    def infer(self):
        self.logger.info(' inference process...')

        if (self.args.dataset == 'CUFED'):
            self.model.eval()
            with torch.no_grad():
                for image_batch, sample_batched in enumerate(self.dataloader['infer']):
                    sample_batched = self.prepare(sample_batched)
                    low_resolution_image = sample_batched[
                        'low_resolution_image']
                    upsampled_low_resolution_image = sample_batched[
                    ...
                    super_resolution, _, _, _, _ = self.model(
                        ...
                        high_resolution_images_as_references=high_resolution_images_as_references,
                        down_and_upsampled_Ref_image=down_and_upsampled_Ref_image)
                    if (self.args.infer_save_results):
                        super_resolution_save = (super_resolution + 1.) * 127.5
                        super_resolution_save = np.transpose(
                            super_resolution_save.squeeze().round().cpu().numpy(),
                            (1, 0))
                        save_path = os.path.join(self.args.save_dir, str(image_batch).zfill(5) + '.obj')
                        cloud_save(save_path, super_resolution_save)
        self.logger.info('Inference over.')
