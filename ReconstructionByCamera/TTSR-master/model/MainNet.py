...
def conv1x1(in_channels, out_channels, stride=1):
    return torch_neural_network.Conv2d(in_channels, out_channels, kernel_size=1,
...
class Backbone(torch_neural_network.Module):
    def __init__(self, number_residual_blocks, number_features, residual_scale):
        ...
        self.conv_head = conv3x3(3, number_features)
...
class MergeTail(torch_neural_network.Module):
    def __init__(self, number_features):
        ...
        self.conv_tail2 = conv1x1(number_features // 2, 3)
...
class MainNet(torch_neural_network.Module):
    def __init__(self, number_residual_blocks, number_features, residual_scale):
        ...
        self.number_residual_blocks = number_residual_blocks  ### a list containing number of resblocks of different stages
    ...
    def forward(self, stage_output, Soft_attention_map=None,
        ...
        ### soft-attention
        stage_1_1_residual = stage_1_1
        stage_1_1_residual = torch.cat((stage_1_1_residual, Hard_attention_output_lv3),
                                       dim=1)
        stage_1_1_residual = self.conv11_head(
            stage_1_1_residual)  # F.relu(self.conv11_head(x11_res))
        stage_1_1_residual = stage_1_1_residual * Soft_attention_map
        stage_1_1 = stage_1_1 + stage_1_1_residual

        stage_1_1_residual = stage_1_1

        for i in range(self.number_residual_blocks[1]):
            stage_1_1_residual = self.residual_block_11[i](stage_1_1_residual)
        stage_1_1_residual = self.conv11_tail(stage_1_1_residual)
        stage_1_1 = stage_1_1 + stage_1_1_residual

        ### stage21, 22
        stage_2_1 = stage_1_1
        stage_2_1_residual = stage_2_1
        stage_2_2 = self.conv12(stage_1_1)
        stage_2_2 = torch_neural_network_functional.relu(
            self.pixelshuffle_12(stage_2_2))

        ### soft-attention
        stage_2_2_residual = stage_2_2
        stage_2_2_residual = torch.cat(
            (stage_2_2_residual, Hard_attention_output_lv2),
            dim=1)
        stage_2_2_residual = self.conv22_head(
            stage_2_2_residual)  # F.relu(self.conv22_head(x22_res))
        stage_2_2_residual = stage_2_2_residual * torch_neural_network_functional.interpolate(
            Soft_attention_map, scale_factor=2, mode='bicubic') # linear
        stage_2_2 = stage_2_2 + stage_2_2_residual

        stage_2_2_residual = stage_2_2

        stage_2_1_residual, stage_2_2_residual = self.cross_scale_feature_intefration_12_output(
            stage_2_1_residual, stage_2_2_residual)

        for i in range(self.number_residual_blocks[2]):
            stage_2_1_residual = self.residual_block_21[i](stage_2_1_residual)
            stage_2_2_residual = self.residual_block_22[i](stage_2_2_residual)

        stage_2_1_residual = self.conv21_tail(stage_2_1_residual)
        stage_2_2_residual = self.conv22_tail(stage_2_2_residual)
        stage_2_1 = stage_2_1 + stage_2_1_residual
        stage_2_2 = stage_2_2 + stage_2_2_residual

        ### stage31, 32, 33
        stage_3_1 = stage_2_1
        stage_3_1_residual = stage_3_1
        stage_3_2 = stage_2_2
        stage_3_2_residual = stage_3_2
        stage_3_3 = self.conv23(stage_2_2)
        stage_3_3 = torch_neural_network_functional.relu(
            self.pixelshuffle_23(stage_3_3))

        ### soft-attention
        stage_3_3_residual = stage_3_3
        stage_3_3_residual = torch.cat(
            (stage_3_3_residual, Hard_attention_output_lv1),
            dim=1)
        stage_3_3_residual = self.conv33_head(
            stage_3_3_residual)  # F.relu(self.conv33_head(x33_res))
        stage_3_3_residual = stage_3_3_residual * torch_neural_network_functional.interpolate(
            Soft_attention_map, scale_factor=4, mode='bicubic')
        stage_3_3 = stage_3_3 + stage_3_3_residual

        stage_3_3_residual = stage_3_3

        stage_3_1_residual, stage_3_2_residual, stage_3_3_residual = self.cross_scale_feature_intefration_123_output(
            stage_3_1_residual, stage_3_2_residual, stage_3_3_residual)

        for i in range(self.number_residual_blocks[3]):
            stage_3_1_residual = self.residual_block_31[i](stage_3_1_residual)
            stage_3_2_residual = self.residual_block_32[i](stage_3_2_residual)
            stage_3_3_residual = self.residual_block_33[i](stage_3_3_residual)

        stage_3_1_residual = self.conv31_tail(stage_3_1_residual)
        stage_3_2_residual = self.conv32_tail(stage_3_2_residual)
        stage_3_3_residual = self.conv33_tail(stage_3_3_residual)
        stage_3_1 = stage_3_1 + stage_3_1_residual
        stage_3_2 = stage_3_2 + stage_3_2_residual
        stage_3_3 = stage_3_3 + stage_3_3_residual
