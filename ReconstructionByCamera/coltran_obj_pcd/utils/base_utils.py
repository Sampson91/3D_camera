...
def get_blackwhite_and_color(inputs, colorspace):
...
def convert_bits(bits_convert, number_of_bits_out=8, number_of_bits_in=8):
    """Quantize / dequantize from n_bits_in to n_bits_out."""
    if number_of_bits_in == number_of_bits_out:
        return bits_convert
    bits_convert = tf.cast(bits_convert, dtype=tf.float32)
    bits_convert = bits_convert / 2 ** (number_of_bits_in - number_of_bits_out)
    bits_convert = tf.cast(bits_convert, dtype=tf.int32)
    return bits_convert
...
def labels_to_bins(labels, number_of_symbols_per_channel):
    """Maps each (R, G, B) channel triplet to a unique bin.

    Args:
      labels: 4-D Tensor, shape=(batch_size, H, W, 3).
      number_of_symbols_per_channel: number of symbols per channel.

    Returns:
      labels: 3-D Tensor, shape=(batch_size, H, W) with 512 possible symbols.
    """
    labels = tf.cast(labels, dtype=tf.float32)
    channel_hash = [number_of_symbols_per_channel ** 2,
                    number_of_symbols_per_channel, 1.0]
...
def bins_to_labels(bins, number_of_symbols_per_channel):
    ...
    """
    labels = []
    factor = int(number_of_symbols_per_channel ** 2)
    ...
    for _ in range(3):
        ...
        factor = factor // number_of_symbols_per_channel
