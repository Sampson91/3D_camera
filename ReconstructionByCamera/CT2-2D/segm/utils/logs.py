...
def plot_logs(logs, x_key, y_key, size, vmin, vmax, epochs):
    ...
    for name, log in logs.items():
        ...
        minimum = np.min((minimum, min(log_plot)))
        maximum = np.max((maximum, max(log_plot)))
    if vmin is not None:
        minimum = vmin
    if vmax is not None:
        maximum = vmax
    delta = 0.1 * (maximum - minimum)
    ...
    # plot parameters
    fig, axial = plt.subplots(figsize=figsize)
    axial.set_xlabel(x_key)
    axial.set_ylabel(y_key)
    ...
    colors = plot_lines(axial, domains, lines, labels_logs)
    axial.grid(True, alpha=0.5)
    axial.set_ylim(minimum - delta, maximum + delta)
...
def print_logs(logs, x_key, y_key, last_log_index=None):
    delimiter = "   "
    string_information = ""
    keys = []
    y_keys = y_key.split("/")
    for name, log in logs.items():
        ...
        if log_index is None:
            log_index = len(log) - 1
        while y_keys[0] not in log[log_index]:
            log_index -= 1
        last_log = log[log_index]
        log_x = last_log[x_key]
        log_y = last_log[y_keys[0]]
        for y_key in y_keys[1:]:
            log_y = log_y[y_key]
        string_information += f"{name}:\n"
        # s += f"{delim}{x_key}: {log_x}\n"
        string_information += f"{delimiter}{y_key}: {log_y:.4f}\n"
        keys += list(last_log.keys())
    keys = list(set(keys))
    keys = ", ".join(keys)
    string_information = f"keys: {keys}\n" + string_information
    print(string_information)
    ...
    for name, path in logs_path.items():
        ...
        with open(path, "r") as file:
            for line in file.readlines():
                dictionary_line = json.loads(line)
                logs[name].append(dictionary_line)
    return logs
