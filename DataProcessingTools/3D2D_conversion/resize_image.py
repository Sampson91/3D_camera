...
def copy_file(original_path_with_file, destination_path):  # copy file function
    if not os.path.isfile(original_path_with_file):
        ...
        file_path, file_name = os.path.split(
            original_path_with_file)  # split file name and path
        if not os.path.exists(destination_path):
            os.makedirs(destination_path)  # 创建路径
        shutil.copy(original_path_with_file,
                    destination_path + '/' + file_name)  # 复制文件
...
def resize_image_function(image_input, image_output, pixel):
    ...
    # Image.ANTIALIAS：高质量
    file_path, file_name = os.path.split(image_input)
    image_output_file = os.path.join(image_output, file_name)
    out.save(image_output_file, type)
